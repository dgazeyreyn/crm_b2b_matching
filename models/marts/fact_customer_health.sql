with
    ...
    acv as (select * from {{ ref("stg_crm_tenants__annual_contract_value") }}),

    nps as (select * from {{ ref("stg_crm_tenants__data_quality_nps") }}),

    industry as (select * from {{ ref("stg_crm_tenants__industries") }}),
