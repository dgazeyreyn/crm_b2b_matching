...
support as (
  select
    ...
    sum(case when feedback_theme in ('match_accuracy','wrong_company_matched','no_match_found') then frequency else 0 end) as match_issue_tickets
...
tenant_matches as (
  select
    ...
    case when m.validation_mismatch then 1 else 0 end as validation_incorrect_flag
  from matches m
  left join {{ ref('dim_crm_account') }} c on m.crm_account_id = c.crm_account_id
),
...
select
  {{ dbt_utils.generate_surrogate_key(['t.tenant_id']) }} as tenant_quality_id,
  t.tenant_id,
  t.total_matches,
  t.low_confidence_count,
  safe_divide(t.low_confidence_count, nullif(t.total_matches,0)) as pct_low_confidence,
  t.region_mismatch_count,
  safe_divide(t.region_mismatch_count, nullif(t.total_matches,0)) as pct_region_mismatch,
  t.validated_count,
  safe_divide(t.validation_incorrect_count, nullif(t.validated_count,0)) as validation_incorrect_rate,
  coalesce(s.total_support_tickets,0) as total_support_tickets,
  coalesce(s.match_issue_tickets,0) as match_issue_tickets,
  case when coalesce(s.total_support_tickets,0) > 0
       then safe_divide(coalesce(s.match_issue_tickets,0), s.total_support_tickets)
       else null end as pct_tickets_match_issues
from tenant_agg t
