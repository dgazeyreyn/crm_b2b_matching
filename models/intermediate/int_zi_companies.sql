with companies as (
    ...
    select * from {{ ref('stg_zi_companies__companies') }}
),

urls as (

    select * from {{ ref('stg_zi_companies__urls') }}
),

headcount as (

    select * from {{ ref('stg_zi_companies__headcount') }}
),

locations as (

    select * from {{ ref('stg_zi_companies__locations') }}
),
